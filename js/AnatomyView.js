// ✅ Use CDN imports so it works directly with Live Server
...
class AnatomyView {
        ...
        loader.load(
            ...
            (gltf) => {
                ...
                if (modelType === "skeleton") {
                    model.scale.set(1.8, 1.8, 1.8); // Increased from 0.01 to 1.8
                    model.position.set(0, -4, 0);   // Adjusted Y position slightly
                } else if (modelType === "scoliosis") {
                    model.scale.set(0.01, 0.01, 0.01); // Kept the original scale for this model
                    model.position.set(0, -1.5, 0);
                } else { // This is for the 'male' model
            ...
            (progress) => {
                if (loadingIndicator) {
                    ...
                    loadingIndicator.querySelector("div:last-child").textContent =
                        `Loading... ${percent}%`;
                }
            },
            (error) => {
    ...
    updateModelInfo() {
        ...
        document.getElementById("currentModel").textContent =
            modelNames[this.currentModelType] || "Unknown";

        document.getElementById("polyCount").innerHTML =
            `${Math.floor(this.polyCount)}<span class="metric-unit"> faces</span>`;

        document.getElementById("modelInfo").innerHTML =
            `<div class="pressure-item"><span>Wireframe</span>
              <span class="pressure-value">${this.isWireframe ? "On" : "Off"}</span></div>`;
    }
    ...
    selectModel(modelType) {
        document.querySelectorAll(".control-panel .control-button")
    ...
    toggleWireframe(forceUpdate = false) {
        ...
        document.getElementById("wireframeBtn")
            .classList.toggle("active", this.isWireframe);
