...
class SpinalPressureVisualizer {
  ...
  createFallbackGeometry() {
    ...
    const torso = new THREE.Mesh(
      new THREE.CylinderGeometry(0.6, 0.8, 2.5, 16),
      bodyMaterial
    );
  ...
  generateSimulatedData() {
    ...
    for (let y = 0; y < this.gridHeight; y++) {
      for (let x = 0; x < this.gridWidth; x++) {
        const dist =
          Math.abs(x - this.gridWidth / 2) + Math.abs(y - centerY) * 0.5;
        const falloff = Math.max(0, 1 - dist * 0.3);
        const wave =
          (Math.sin(2 * time + x * 0.5) + Math.cos(1.5 * time + y * 0.7)) * 0.1;
  ...
  updateMetrics() {
    ...
    if (maxEl)
      maxEl.innerHTML = `${this.maxPressure.toFixed(
        1
      )}<span class="metric-unit">kPa</span>`;
    if (avgEl)
      avgEl.innerHTML = `${this.avgPressure.toFixed(
        1
      )}<span class="metric-unit">kPa</span>`;
    if (activeEl)
      activeEl.innerHTML = `${this.activeSensors}<span class="metric-unit">/ 64</span>`;
    ...
    if (pointsEl) {
      ...
      if (this.topPressurePoints.length === 0) {
        ...
        this.topPressurePoints.forEach((p) => {
          ...
          div.innerHTML = `<span class="pressure-location">${p.location}</span>
                           <span class="pressure-value">${p.value.toFixed(
                             1
                           )} kPa</span>`;
